...
mycursor.execute(
    "CREATE TABLE mentor(mentor_id INT PRIMARY KEY AUTO_INCREMENT, mentor_fname NVARCHAR(100), mentor_lname NVARCHAR(30), mentor_email NVARCHAR(100), is_admin INT, password BLOB, salt BLOB)")
...
mycursor.execute(
    "CREATE TABLE user(user_id INT PRIMARY KEY AUTO_INCREMENT, username VARCHAR(100), password BLOB, salt BLOB)")
